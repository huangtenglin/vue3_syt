    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="userStore.loginDialog = false">确定</el-button>
          <el-button type="primary" @click="changeDialog(false)">
            取消
          </el-button>
        </div>
      </template>
...
let form = ref<any>();
//收集表单数据---手机号码
let loginParam = reactive<RuleForm>({
  phone: '', //收集手机号码
  code: '', //收集验证码
});
// 计算出当前单元素搜集是否手机格式
...
const handleClose = () => {
  userStore.changeLoginDialog(false);
};
const changeDialog = (type: boolean) => {
  userStore.changeLoginDialog(type);
};
